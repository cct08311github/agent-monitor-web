...
// 導入安全學習系統
const { SecureWebLearner } = require('./secure_web_learner_fixed');
...
// 初始化安全學習器
const secureLearner = new SecureWebLearner();
...
app.get('/api/learn/search', async (req, res) => {
  try {
    ...
    const results = await secureLearner.searchSafe(query);
...
app.get('/api/learn/topic', async (req, res) => {
  try {
    ...
    const learnings = await secureLearner.learnFromTopic(topic);
