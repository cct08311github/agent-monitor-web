...
describe('cronPins', () => {
  ...
  it('togglePin adds an id when not pinned', () => {
    const result = togglePin('cron-1')
    expect(result).toContain('cron-1')
    expect(loadPins()).toContain('cron-1')
  })

  it('togglePin removes an id when already pinned', () => {
    togglePin('cron-1')
    const result = togglePin('cron-1')
    expect(result).not.toContain('cron-1')
    expect(loadPins()).not.toContain('cron-1')
  })
