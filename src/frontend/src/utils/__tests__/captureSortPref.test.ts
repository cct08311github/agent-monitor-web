...
import { isValidOrder, loadSortOrder, saveSortOrder } from '../captureSortPref'
...
describe('captureSortPref', () => {
  ...
  // ── loadSortOrder — defaults ───────────────────────────────────────────────

  describe('loadSortOrder — default', () => {
    it('returns "desc" when localStorage is empty', () => {
      expect(loadSortOrder()).toBe('desc')
    })

    it('returns "desc" for an invalid stored value', () => {
      ctx.store['oc_capture_sort_order'] = 'invalid'
      expect(loadSortOrder()).toBe('desc')
    })

    it('returns "desc" for a numeric-string stored value', () => {
      ctx.store['oc_capture_sort_order'] = '42'
      expect(loadSortOrder()).toBe('desc')
    })
  })

  // ── loadSortOrder — valid stored values ────────────────────────────────────

  describe('loadSortOrder — valid stored values', () => {
    it('returns "asc" when "asc" is stored', () => {
      ctx.store['oc_capture_sort_order'] = 'asc'
      expect(loadSortOrder()).toBe('asc')
    })

    it('returns "desc" when "desc" is stored', () => {
      ctx.store['oc_capture_sort_order'] = 'desc'
      expect(loadSortOrder()).toBe('desc')
    })
  })

  // ── loadSortOrder — exception path ────────────────────────────────────────

  describe('loadSortOrder — localStorage exception', () => {
  ...
  // ── saveSortOrder + loadSortOrder round-trip ───────────────────────────────

  describe('saveSortOrder + loadSortOrder round-trip', () => {
  ...
  // ── saveSortOrder — exception path ────────────────────────────────────────

  describe('saveSortOrder — localStorage exception', () => {
    it('does not throw when localStorage.setItem throws', () => {
      ctx.throwOnAccess = true
      expect(() => saveSortOrder('asc')).not.toThrow()
    })
  })
})
